...
const userNotification = {userType: 'admin', audioId: null, videoId: null, read: false }
...
describe("User signup, login", () => {
  ...
  describe("New Audio & Notification", ()=>{
    it('Should add new audio', async ()=> {
        const response = await request(app).post('/api/audio')
            .send(audio)
      ...
      it('Should add new notification', async () => {
          const response = await request(app).post('/api/notification/add')
              .send(notification)
              .set('Authorization', `Bearer ${user.token}`)
              .expect(201)
      ...
      it('Should add UserNotification', async () => {
          Object.assign(userNotification, { users: [user._id], audioId: audio._id, notification: notification._id })
          const response = await request(app).post('/api/usernotification')
